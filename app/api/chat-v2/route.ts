...
6. Para metas/objetivos, use 'listarObjetivos' imediatamente para ver o saldo e progresso das metas. Se for criar um novo objetivo ('criarObjetivo'), registrar aporte ('registrarAporteObjetivo') ou excluir um objetivo ('deletarObjetivo'), peça a confirmação do usuário (Regra 2) antes de executar. Para excluir ou aportar, primeiro use 'listarObjetivos' para encontrar o ID correto se necessário.
...
11. DIA ÚTIL OU NÃO (CRÍTICO): Sempre que o usuário solicitar o cadastro de uma conta fixa (receita ou despesa recorrente), se ele NÃO especificar explicitamente se o dia é um "dia útil" ou um "dia corrido" (dia normal do calendário), você NÃO deve cadastrar a conta ainda e NÃO deve apresentar a pergunta de confirmação ("Você confirma?" ou "Posso confirmar?"). Em vez disso, você deve perguntar explicitamente a ele se o vencimento/recebimento será em um "dia útil" ou em um "dia corrido" (ex: "Entendido, e esse vencimento será no dia 10 útil ou dia 10 corrido?"). Só execute o fluxo de confirmação e o cadastro após ele responder a essa pergunta.
12. COMPROMISSO DE METAS (CRÍTICO): Ao criar um novo objetivo ou meta financeira ('criarObjetivo'), se o usuário escolher a frequência mensal ('monthly') ou semanal ('weekly') e NÃO especificar explicitamente o dia do mês ou dia da semana (ex: "todo dia 10", "toda segunda-feira"), você NÃO deve chamar a ferramenta de criação e NÃO deve pedir a confirmação dele ("Você confirma?" ou "Posso confirmar?"). Em vez disso:
    - Se for mensal ('monthly'), pergunte explicitamente em qual dia do mês ele pretende fazer os aportes (ex: "Em qual dia do mês você quer guardar o valor?").
    - Se for semanal ('weekly'), pergunte explicitamente em qual dia da semana ele pretende fazer os aportes (ex: "Em qual dia da semana você quer guardar o valor?").
    Só execute o fluxo de confirmação e criação após ele responder.
...
- Escreva a resposta em texto para o usuário APENAS no passo final, depois que todas as ferramentas necessárias tiverem retornado seus resultados. Isso garante que a resposta final seja limpa, coesa e sem contradições.`,
    ...
    tools: {
      ...
      deletarObjetivo: tool({
        description: 'Exclui um objetivo/meta financeira existente no banco de dados usando o ID do objetivo.',
        inputSchema: z.object({
          goal_id: z.string().describe('O ID (UUID) único do objetivo/meta a ser excluído. Deve ser obtido previamente consultando os objetivos.')
        }),
        execute: async ({ goal_id }: { goal_id: string }) => {
          console.log("Finchat API [deletarObjetivo] requested for ID:", goal_id);
          const { error } = await supabase
            .from('goals')
            .delete()
            .eq('id', goal_id)
          ...
          if (error) {
            return { success: false, message: 'Erro ao excluir objetivo: ' + error.message };
          }
